...
const maximumFutureLifetimeMs = 367 * 24 * 60 * 60 * 1000

const normalizedEmail = (value) => typeof value === 'string' ? value.trim().toLowerCase() : ''

export function newBalloonProposalResponseExpiry(validUntil) {
  if (typeof validUntil !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(validUntil)) return null
  const expiry = new Date(`${validUntil}T23:59:59.999Z`)
  return Number.isFinite(expiry.getTime()) ? expiry : null
}

export function signNewBalloonProposalCapability({ proposalId, quoteRequestId, buyerEmail, expiresAt, secret }) {
  const email = normalizedEmail(buyerEmail)
  const expiryMs = new Date(expiresAt).getTime()
  if (!uuidPattern.test(String(proposalId || '')) || !uuidPattern.test(String(quoteRequestId || ''))) return null
  ...
  const signature = createHmac('sha256', secret)
    .update(`new-balloon-proposal-response|v1|${proposalId}|${quoteRequestId}|${email}|${expirySeconds}`)
    .digest('hex')
  return `${expirySeconds}.${signature}`
}

export function verifyNewBalloonProposalCapability(input, now = new Date()) {
  ...
  const expected = signNewBalloonProposalCapability({ ...input, expiresAt: new Date(expiryMs) })
