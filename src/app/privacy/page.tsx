  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-16 sm:py-24">
      ...
      <div className="prose prose-slate dark:prose-invert lg:prose-lg max-w-none">
        ...
        <p>When you register for an AeroTrade account, we collect your name, email address, and optionally your phone number. Registration alone does not subscribe you to the bi-weekly marketplace newsletter. That optional marketing update is sent only after an explicit choice, its current consent state and timestamps are retained as evidence, and every message includes a signed stop link. You can also change the same preference from your dashboard; stopping the newsletter does not close your account or operational alerts requested separately. When you submit an enquiry, record wanted equipment, watch a specific listing or request help preparing a sale, we store the contact details, requirements, consent choices, relevant listing or equipment category, delivery evidence and commercial status. A listing watch remains inactive until you confirm it by email; it records its confirmation, current state and notification evidence and includes a signed unsubscribe action. It is used only for material changes to that listing and not for general marketing. A seller may receive a transactional request to review whether active adverts are still available. Its private 14-day link is bound to that seller, email and exact evidence cycle, is not indexed, and only records dated per-advert availability after the seller reviews the list, checks an explicit declaration and submits the form; opening the email or page changes nothing. An assisted seller may also provide the public URL of an advert on another website so AeroTrade can review the information manually; AeroTrade does not automatically copy or publish that external content. If a buyer adds a non-binding price indication, or either party accepts negotiation, counters or declines through AeroTrade, we retain the amount, response, relationship to the preceding response and email-delivery status as private commercial evidence. A buyer response link is signed for that buyer, enquiry and seller response, expires after 30 days and accepts one response to that seller update. A separate signed status link can display that buyer&apos;s private enquiry and negotiation history for up to 90 days without requiring an account; it is bound to the enquiry and buyer email and is not indexed. These records do not reserve equipment, execute payment or create a sale contract. Assisted-sale requests remain private and do not create or publish a listing. When you subscribe to annual Buyer Early Access or pay once for Seller Launch Promotion, your payment information is processed by Stripe; we do not store your card details on our servers. We retain a private checkout-status record so an interrupted checkout can be resumed without creating another account; it contains no card data or checkout URL.</p>
        <p>A visitor without an account can request the same optional marketplace newsletter from the public website. AeroTrade stores the normalized email privately and a one-way HMAC request key for rate limiting, but no raw IP address or browser identifier. We may also retain the closed website entry point, bounded campaign labels, external referrer host and a daily server-HMAC journey key so aggregate acquisition can be compared with later marketplace activity; no full referrer URL or raw visitor identifier is stored. The request remains inactive until the email provider accepts the confirmation message and the visitor explicitly confirms on the private page; merely opening the email or page does nothing. We retain the request, confirmation and unsubscribe timestamps as consent evidence, deduplicate the address against account subscribers, and include an immediate signed stop link in every newsletter.</p>
        ...
        <p>If a seller requests an AeroTrade listing verification, the platform stores the review state, the closed category of identity and supporting evidence reviewed, the decision reason where applicable, and audit timestamps and actors. The seller receives a transactional, reply-enabled checklist for handing the necessary evidence directly to the AeroTrade administrator. This workflow does not upload or retain copies of identity documents, aircraft records, document numbers or evidence links in the marketplace database. Original evidence is reviewed outside the platform until a separate, approved retention process exists.</p>
        ...
        <ul>
          ...
          <li>To send the optional marketplace newsletter only after a public visitor or account holder completes the applicable explicit confirmation flow, and to suppress it immediately after a valid stop request.</li>
