...
export default function SellerAssistanceForm({ defaultEmail = '', sourceContext = 'sell_gateway' }: { defaultEmail?: string; sourceContext?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border bg-card p-6 shadow-sm sm:p-8">
      ...
      <label className="block space-y-1.5 text-sm font-medium">Already advertised on another website?<input name="existing_listing_url" type="url" inputMode="url" maxLength={1000} placeholder="Paste the public advert URL and AeroTrade can help transfer the information" className="w-full rounded-lg border bg-background px-3 py-2" /><span className="block text-xs font-normal text-muted-foreground">We record the link for private review; nothing is copied or published automatically.</span></label>
