...
export default async function AssistedSalePage({
  ...
  const sourceContext = normalizeSellerAcquisitionSource(params.source, 'sell_gateway')
    ...
    <main className="bg-secondary/30">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8 lg:py-16">
        <section className="flex flex-col justify-center">
          <Link href="/sell?source=assisted_conversion" className="mb-6 inline-flex w-fit items-center gap-2 text-sm font-semibold text-primary">Create the full listing yourself <ArrowRight className="h-4 w-4" /></Link>
          <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl">Want to sell, but not ready to list?</h1>
          <p className="mt-5 text-lg leading-8 text-muted-foreground">Do not abandon the sale because the photos, documents, description or price are not ready. Record the opportunity privately, or paste an existing public advert, and AeroTrade can help you prepare the same normal marketplace listing.</p>
        ...
        <SellerAssistanceForm defaultEmail={user?.email || ''} sourceContext={sourceContext} />
      </div>
    </main>
  )
}
