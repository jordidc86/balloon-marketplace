...
const checks = [
  ...
  {
    name: 'A complete marketplace transaction is rehearsed without production or external messages',
    file: 'scripts/lib/marketplace-transaction-rehearsal.mjs',
    required: [
      'record_seller_inquiry_response',
      'record_buyer_inquiry_response',
      'close_listing_by_actor',
  ...
  {
    name: 'Negotiation freshness uses monotonic database order rather than timestamp and UUID ties',
    file: 'supabase/migrations/20260831730000_negotiation_event_order_integrity.sql',
    required: ['event_sequence bigint generated by default as identity', 'marketplace_inquiry_offer_events_sequence_idx', 'order by event.event_sequence desc', 'record_buyer_inquiry_response', 'This negotiation link is no longer current', 'grant execute on function public.record_buyer_inquiry_response', 'monotonic database sequence'],
    forbidden: ['order by event.created_at desc, event.id desc'],
  },
  {
