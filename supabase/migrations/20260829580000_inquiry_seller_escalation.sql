-- Add one internal recovery signal for an open marketplace enquiry whose seller
-- has not advanced it 48 hours after a provider-accepted seller reminder.
-- This extends the existing private delivery ledger and never contacts the buyer.
...
comment on constraint commercial_notification_receipts_notification_type_check on public.commercial_notification_receipts is
  'Closed transactional-delivery vocabulary. inquiry_seller_escalation is one internal admin signal after an accepted seller reminder has remained unresolved for 48 hours.';
