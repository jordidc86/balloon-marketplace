-- Give every stored buying requirement a durable, retryable transactional
-- acknowledgement without treating it as marketing consent.
...
alter table public.commercial_notification_receipts
  add constraint commercial_notification_receipts_notification_type_check
    check (notification_type in (
      'listing_created_admin','quote_created_admin','wanted_request_admin','wanted_buyer_ack','listing_quality_quarantine',
...
comment on constraint commercial_notification_receipts_notification_type_check on public.commercial_notification_receipts is
  'Closed transactional vocabulary. wanted_buyer_ack confirms only the stored buying requirement and its chosen match-alert preference; it grants no marketing consent.';
