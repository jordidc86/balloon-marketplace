alter table public.listing_availability_confirmations
  drop constraint if exists listing_availability_confirmations_source_check;

alter table public.listing_availability_confirmations
  add constraint listing_availability_confirmations_source_check check (
    source in ('SELLER_DASHBOARD', 'SELLER_EMAIL_CAPABILITY')
  );
...
begin
  ...
  if (select count(distinct listing_id) from unnest(p_listing_ids) as scope(listing_id)) <> cardinality(p_listing_ids) then
...
comment on function public.confirm_listing_availability_from_seller_digest(uuid, text, uuid[]) is
  'Records explicit seller availability from a current provider-accepted private email capability. The bounded listing scope is supplied only after HMAC verification; it creates immutable per-listing evidence and never changes publication, price, payment or ownership.';
