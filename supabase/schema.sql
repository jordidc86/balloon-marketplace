...
create table public.commercial_notification_receipts (
  id uuid default uuid_generate_v4() primary key,
  notification_type text not null check (notification_type in ('listing_created_admin', 'quote_created_admin', 'wanted_request_admin')),
  entity_type text not null check (entity_type in ('listing', 'quote_request', 'wanted_request')),
...
create index wanted_requests_status_created_idx on public.wanted_requests (status, created_at desc);
create index wanted_requests_match_idx on public.wanted_requests (category, currency, budget_max_minor, created_at desc) where status not in ('CLOSED', 'SPAM');
create index wanted_requests_buyer_dedup_idx on public.wanted_requests (lower(buyer_email), category, created_at desc);
create index wanted_requests_submission_rate_idx on public.wanted_requests (submission_key, created_at desc) where submission_key is not null;
create index wanted_requests_attribution_idx on public.wanted_requests (utm_source, status, created_at desc);
create trigger set_wanted_requests_updated_at before update on public.wanted_requests for each row execute procedure public.set_updated_at();
alter table public.wanted_requests enable row level security;
revoke all on public.wanted_requests from anon, authenticated;
create policy "Admins can manage wanted requests" on public.wanted_requests for all to authenticated
  using (exists (select 1 from public.users where id = auth.uid() and role = 'admin'))
  with check (exists (select 1 from public.users where id = auth.uid() and role = 'admin'));
...
create table public.seller_funnel_events (
  id uuid default uuid_generate_v4() primary key,
  event_key text not null unique check (char_length(event_key) = 64),
...
create index seller_funnel_stage_created_idx on public.seller_funnel_events (stage, created_at desc);
create index seller_funnel_seller_created_idx on public.seller_funnel_events (seller_id, created_at desc);
create index seller_funnel_listing_created_idx on public.seller_funnel_events (listing_id, created_at desc) where listing_id is not null;
...
create policy "Admins can read seller funnel" on public.seller_funnel_events for select to authenticated
  using (exists (select 1 from public.users where id = auth.uid() and role = 'admin'));
