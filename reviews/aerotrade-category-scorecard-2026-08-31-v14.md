...
|---|---|---:|---:|---:|---|
| 1 | Value proposition, business model and unit economics | 94 | 30 | 75 | Buyer and seller products have distinct provider evidence; one sold listing has an immutable closure. Both synthetic commercial paths now prove revenue and contribution calculations, but no real marketplace transaction has evidenced complete economics. |
...
| 7 | New-balloon sales: Pasha and Schroeder | 98 | 0 | 69 | Request, proposal delivery, immutable buyer response, outcome, settlement and unit economics now pass a complete disposable-database intermediation rehearsal. Production still has zero genuine requests or proposals. |
...
| 12 | Deployment, production reliability and operability | 99 | 92 | 97 | Netlify publishes only `production`, ordinary `main` pushes cause no deploy, and the candidate passes 212 tests, 231 contracts and a 75-migration disposable recovery plus both transaction rehearsals. Application production is healthy, but the expired external Meta credential prevents claiming end-to-end social operability. |
...
- The two transaction rehearsals improve technical confidence but intentionally do not raise commercial proof: all actors, amounts and evidence references were synthetic and production was neither accessed nor mutated.
...
3. **Commercial closing proof:** both full revenue paths now pass in a disposable database, but no genuine marketplace transaction or new-balloon lead has completed either one.
