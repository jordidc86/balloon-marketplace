...
| 6 | Premium, payments and monetisation | 99 | 70 | 90 | Seller Launch checkout is durable and fail-closed. A fresh read-only Stripe audit confirms two active Premium subscriptions and one unreimbursed 9.99 EUR charge in the 90-day window. That 9 August payment predates the receipt and checkout-intent ledgers but resolves by both account email and subscription to the active entitlement; no seller-promotion or closing revenue is proven. |
...
| 12 | Deployment, production reliability and operability | 99 | 95 | 98 | Netlify publishes only `production`, ordinary `main` pushes cause no deploy, and the current candidate passes 211 tests, 226 contracts, lint, types, build and a disposable 74-migration rehearsal. |
