...
4. **Prove one complete commercial journey.** The grouped candidate now provides four high-intent European entries that reuse current inventory, preserve external attribution and distinguish localized entry from catalogue search in the existing private demand ledger. Target one genuine landing visit progressing to a listing opening and then one watch, enquiry, wanted request or new-balloon request. No synthetic event counts or readiness increase is claimed from the pages alone.
...
A later PII-free read-only snapshot at `2026-08-29T17:35:41.896Z` confirmed that all 12 active listings across 5 sellers still lack owner availability evidence. The grouped candidate now extends the same confirmation system so each seller can confirm every currently due owned advert with one action while retaining one immutable confirmation row per listing. The dashboard path remains authenticated; an accepted grouped email may instead open a private 14-day review bound to seller, email, exact evidence cycle and expiry. Opening the page is read-only, inventory drift invalidates it and confirmation requires an explicit declaration plus POST. The current portfolio would therefore require 5 seller actions instead of 12. This does not count as availability proof until those sellers actually confirm; no email or database write was performed during the audit.
