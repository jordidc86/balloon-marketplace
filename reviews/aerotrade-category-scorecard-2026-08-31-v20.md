# AeroTrade category scorecard — 2026-08-31 v20

This update uses the fresh read-only snapshot `marketplace-audit-2026-08-31-v16.json`, existing Stripe and provider evidence, both disposable-database transaction rehearsals, and the candidate verifications for inventory feed, seller availability, exact consent, seller distribution and schema-bound production promotion. Candidate capability is not counted as commercial proof. The scorecard contains no personal data.
...
| 12 | Deployment, production reliability and operability | 99 | 92 | 97 | The candidate passes 231 tests, 242 contracts, TypeScript, ESLint, optimized build and a 77-migration disposable recovery. Promotion is now bound to five exact append-only migrations, their content fingerprint and authoritative remote-ledger readback before one application deploy. It remains unpromoted. |
...
- Deployment risk decreased materially, but commercial proof remains unchanged because no real view, enquiry, seller confirmation, payment or marketplace transaction was manufactured.

## Next highest-value constraints

1. **Consolidated production activation:** the candidate is now safely promotable only as schema first, verified readback second and one application deploy last. Five exact migrations remain unapplied pending explicit release approval.
